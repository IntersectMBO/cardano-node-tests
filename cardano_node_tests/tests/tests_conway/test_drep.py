...
class TestDReps:
    ...
    def test_drep_id_is_blake2b_224_of_drep_vkey(
        ...
        * Check drep ID generated from cli is same as blake2b_224 hash of drep vkey
        """
        reqc.cli085.start(url=helpers.get_vcs_link())
...
class TestNegativeDReps:
    ...
    def test_no_multiple_delegation(
        ...
        * check that the Drep certificate placed at last of the certificates is delegated to
        ...
        reqc.cli087.start(url=helpers.get_vcs_link())
    ...
    def test_no_delegation_without_stake_registration(
        ...
        * Expect error StakeKeyNotRegisteredDELEG
        ...
        reqc.cli088.start(url=helpers.get_vcs_link())
    ...
    def test_drep_no_retirement_before_register(
        ...
        * check it is not possible to retire before register
        ...
        reqc.cli089.start(url=helpers.get_vcs_link())
        ret_cert = cluster.g_conway_governance.drep.gen_retirement_cert(
            cert_name=temp_template,
            deposit_amt=deposit,
            drep_vkey_file=drep_keys.vkey_file,
        ...
        if submit_method == "api":
            with pytest.raises(submit_api.SubmitApiError) as excinfo_submit:
                clusterlib_utils.build_and_submit_tx(
                    cluster_obj=cluster,
                    name_template=f"{temp_template}_ret",
                    src_address=payment_addr.address,
            ...
            err_msg = str(excinfo_submit.value)
            err_msg = err_msg.replace("\\", "")
        else:
            with pytest.raises(clusterlib.CLIError) as excinfo_cli:
                clusterlib_utils.build_and_submit_tx(
                    cluster_obj=cluster,
                    name_template=f"{temp_template}_ret",
                    src_address=payment_addr.address,
            ...
            err_msg = str(excinfo_cli.value)

        assert f'ConwayDRepNotRegistered (KeyHashObj (KeyHash "{drep_id}' in err_msg, err_msg
        reqc.cli089.success()
    ...
    def test_drep_no_multiple_registration(
        ...
        * Expect ConwayDRepAlreadyRegistered on the second time
        ...
        reqc.cli090.start(url=helpers.get_vcs_link())
        ...
        # Expecting error for both cases as drep is already registered
        if submit_method == "api":
            with pytest.raises(submit_api.SubmitApiError) as excinfo_submit:
                clusterlib_utils.build_and_submit_tx(
                    cluster_obj=cluster,
        ...
        assert f'ConwayDRepAlreadyRegistered (KeyHashObj (KeyHash "{drep_id}' in err_msg, err_msg
        reqc.cli090.success()
...
class TestDelegDReps:
    ...
    def test_change_delegation(
        ...
        * Check vote delegation is updated to second drep
        ...
        reqc.cli086.start(url=helpers.get_vcs_link())
