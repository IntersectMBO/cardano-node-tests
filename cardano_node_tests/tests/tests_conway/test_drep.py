    ...
    def test_drep_inactivity(  # noqa: C901
        ...
        * Check DRep activity records using saved DRep status data.
        ...
        def _update_drep_activity(governance_data: governance_setup.DefaultGovernance) -> None:
            ...
            anchor_url = f"http://www.pparam-action-{rand_str}.com"
        ...
        def _check_drep_records() -> tp.List[blockers.GH]:
            ...
            drep1_init_expiry = drep1_state[0][2][0][1]["expiry"]
            assert drep1_init_expiry > drep1_state[0][0] + 5, "Unexpected DRep1 init expiry"
            assert (
                drep1_state[1][2][0][1]["expiry"] > drep1_init_expiry
            ), "DRep1 expiry was not updated"

            assert governance_utils.is_drep_active(
                cluster_obj=cluster, drep_state=drep1_state[-2][2], epoch=drep1_state[-2][0]
            ), "DRep1 is not active"
            assert not governance_utils.is_drep_active(
                cluster_obj=cluster, drep_state=drep1_state[-1][2], epoch=drep1_state[-1][0]
            ), "DRep1 is still active"

            drep2_init_expiry = drep2_state[0][2][0][1]["expiry"]
            assert drep2_init_expiry < drep2_state[0][0] + 3, "Unexpected DRep2 init expiry"
            assert (
                drep2_state[1][2][0][1]["expiry"] > drep2_init_expiry
            ), "DRep2 expiry was not updated"
            assert not governance_utils.is_drep_active(
                cluster_obj=cluster, drep_state=drep2_state[-3][2], epoch=drep2_state[-3][0]
            ), "DRep2 is still active"
            assert not governance_utils.is_drep_active(
                cluster_obj=cluster, drep_state=drep2_state[-2][2], epoch=drep2_state[-2][0]
            ), "DRep2 is still active"
            assert governance_utils.is_drep_active(
                cluster_obj=cluster, drep_state=drep2_state[-1][2], epoch=drep2_state[-1][0]
            ), "DRep2 is not active"
            if drep2_state[-2][2][0][1]["expiry"] > drep2_state[-3][2][0][1]["expiry"]:
