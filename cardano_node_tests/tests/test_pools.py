...
def _create_stake_pool_w_build(
    ...
    return clusterlib.PoolCreationOutput(
        ...
        pool_data=pool_data,
        pool_owners=pool_owners,
        reward_account_vkey_file=pl.Path(
            reward_account_vkey_file or pool_owners[0].stake.vkey_file
        ),
...
def _create_register_pool_delegate_stake_tx(
    ...
    return clusterlib.PoolCreationOutput(
        ...
        pool_owners=pool_owners,
        reward_account_vkey_file=pl.Path(
            reward_account_key_pair.vkey_file
            if reward_account_key_pair
            else pool_owners[0].stake.vkey_file
        ),
        tx_raw_output=tx_raw_output,
    )
...
class TestPoolVoteDeleg:
    ...
    def get_subtests(self) -> tp.Generator[tp.Callable, None, None]:
        ...
        yield same_pool_reward

        def other_pool_reward(
            cluster: clusterlib.ClusterLib, pools: list[clusterlib.PoolCreationOutput]
        ):
            """Test txFeePerByte guardrails defined in the key "0" of default constitution."""
            assert (
