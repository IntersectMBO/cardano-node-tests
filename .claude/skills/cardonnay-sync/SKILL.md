...
5. Now run the actual test. This only exercises the synced ref if the whole
   environment gets rebuilt from `pyproject.toml` and `uv.lock`.
   `runner/regression.sh`, `runner/node_upgrade.sh`, `make test-env` and
   `make install` all do that. A plain `make cluster-scripts && make
   start-cluster` does not. It reuses whatever `./.venv` already exists, so it
   keeps running against the old dependency.
