...
def main():
    ...
    sync_duration_values_dict = ast.literal_eval(str(sync_test_results_dict["sync_duration_per_epoch"]))
    ...
    if not add_bulk_values_into_db(env, col_to_insert, val_to_insert):
        print(f"col_to_insert: {col_to_insert}")
        print(f"val_to_insert: {val_to_insert}")
        exit(1)
